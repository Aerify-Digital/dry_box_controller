...
const uint LCD_BACKLIGHT = 0x08;

const uint LCD_ENABLE_BIT = 0x04;

// By default these LCD display drivers are on bus address 0x27
static uint lcdaddr = 0x3f;
...
void i2c_write_byte(i2c_inst_t *i2c, uint8_t val)
{
    i2c_write_blocking(i2c, lcdaddr, &val, 1, false);
}
...
#define DELAY_US 600
    sleep_us(DELAY_US);
    i2c_write_byte(i2c_default, val | LCD_ENABLE_BIT);
    sleep_us(DELAY_US);
    i2c_write_byte(i2c_default, val & ~LCD_ENABLE_BIT);
    sleep_us(DELAY_US);
}
...
void lcd_send_byte(uint8_t val, int mode)
{
    ...
    i2c_write_byte(i2c_default, high);
    lcd_toggle_enable(high);
    i2c_write_byte(i2c_default, low);
